...
if($user['id']!=$profile['id'] && !checkBS($profile['id'])){
  ...
  <div class="dropdown">
                            ...
                            <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton1">
                                <li><a class="dropdown-item" href="#" data-bs-toggle="modal" data-bs-target="#chatbox" onclick="popchat(<?=$profile['id']?>)"><i class="bi bi-chat-fill"></i> Mesaj at</a></li>
                                <li><a class="dropdown-item " href="assets/php/actions.php?block=<?=$profile['id']?>&username=<?=$profile['username']?>"><i class="bi bi-x-circle-fill"></i> Engelle</a></li>
                            </ul>
                        </div>
    <?php
}
...
<a class="btn btn-sm btn-primary"><i class="bi bi-file-post-fill"></i> <?=count($profile_post)?> Gönderiler</a>
<a class="btn btn-sm btn-primary <?=count($profile['followers'])<1?'disabled':''?>" data-bs-toggle="modal" data-bs-target="#follower_list"><i class="bi bi-people-fill"></i> <?=count($profile['followers'])?> Takipçi</a>
<a class="btn btn-sm btn-primary <?=count($profile['following'])<1?'disabled':''?>" data-bs-toggle="modal" data-bs-target="#following_list"><i class="bi bi-person-fill"></i> <?=count($profile['following'])?> Takip</a>
...
?> 
<button class="btn btn-sm btn-danger unblockbtn" data-user-id='<?=$profile['id']?>' >Engeli kaldır</button>

<?php
}else if(checkBlockStatus($profile['id'],$user['id'])){ ?>
    <div class="alert alert-danger" role="alert">
    <i class="bi bi-x-octagon-fill"></i> @<?=$profile['username']?> seni engelledi !
</div>
   <?php }else if(checkFollowStatus($profile['id'])){
   ?>
<button class="btn btn-sm btn-danger unfollowbtn" data-user-id='<?=$profile['id']?>' >Takibi bırak.</button>
   
   <?php
}else{
    ?>
<button class="btn btn-sm btn-primary followbtn" data-user-id='<?=$profile['id']?>' >Takip et.</button>
...
?>
        ...
        <h3 class="border-bottom">Gönderiler</h3>
        <?php
...
if(checkBS($profile['id'])){
 ...
 <div class="alert alert-secondary text-center" role="alert">
    <i class="bi bi-x-octagon-fill"></i> Gönderiyi görmeye iznin yok !
</div>
   <?php
...
foreach($profile_post as $post){
  ...
  ?> 
                ...
                <div style="font-size:small" class="text-muted">Gönder <?=show_time($post['created_at'])?> </div> 
...
if(count($comments)<1){
    ?>
<p class="p-3 text-center my-2 nce">yorum yok</p>
    <?php
}
...
<div class="d-flex align-items-center p-2">
  ...
  <div class="input-group p-2 border-top">
                            <input type="text" class="form-control rounded-0 border-0 comment-input" placeholder="birşeyler yaz..."
                            ...
                            <button class="btn btn-outline-primary rounded-0 border-0 add-comment" data-cs="comment-section<?=$post['id']?>" data-post-id="<?=$post['id']?>" type="button"
                                id="button-addon2">Yorum yap</button>
                        </div>
                            <?php
                        }else{
                            ?>
<div class="text-center p-2">
yorum yapmak istiyorsanız bu kullanıcıyı takip edin</div>
...
<div class="modal fade" id="following_list" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  ...
  <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Takip Et</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <?php
